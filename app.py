...
# --- 2. 資料讀取 (History模式僅增加年份篩選，邏輯維持您原本的設計) ---
def load_data(dept, semester, grade, history_year=None):
    ...
    try:
        ...
        # === 模式 A: 載入歷史資料 (依您的原始邏輯，僅加入學年度篩選) ===
        if history_year:
            ws_hist = sh.worksheet(SHEET_HISTORY)
            df_hist = get_df(ws_hist)
            if not df_hist.empty:
                for col in ['年級', '學期', '科別']: df_hist[col] = df_hist[col].astype(str)
                ...
                for _, h_row in target_hist.iterrows():
                    ...
                    if not sub_match.empty:
                        # 邏輯：有對應的 submission -> 顯示 submission (使用者修改過的)
...
# --- 4.6 同步歷史資料到 Submission (依 UUID 比對) ---
def sync_history_to_db(dept, history_year):
    ...
    try:
        ...
        target_hist = df_hist[(df_hist['科別'] == dept) & (df_hist['學年度'] == str(history_year))]
        if target_hist.empty: return True

        rows_to_append = []
        for _, row in target_hist.iterrows():
            h_uuid = str(row.get('uuid', '')).strip()
            # 核心邏輯：Submission 有則跳過，沒有則複製
            ...
            row_dict = {
                ...
                "科別": row.get('科別', ''), "學期": str(row.get('學期', '')), "年級": str(row.get('年級', '')), "課程名稱": row.get('課程名稱', ''),
...
def main():
    ...
    if st.session_state.get('loaded'):
        ...
        st.data_editor(
            st.session_state['data'], num_rows="dynamic", use_container_width=True, height=600,
